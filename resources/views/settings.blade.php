...
@section('content')
<div x-data="{showing:'{!! session('settingsOpen') !!}',logoutopen:false}">
...
<div class="profile-cont" x-show="showing=='profilesettings'">
    ...
    <div x-data="{logoutopen:false}" >
        Not {{$user->name}}?<button class="round-btn"  href="#" @click="logoutopen = true">Logout</button>
    ...
    @csrf
        ...
        <input name="username" id="username" type="text" value="{{$user->username}}"
         x-bind:class="{'invalid':username.errorMessage}" data-rules='[]' data-server-errors='{!! json_encode($errors->get('username'))!!}'>
